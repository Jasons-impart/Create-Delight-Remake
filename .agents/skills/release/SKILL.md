...
8. **Artifact transfer on Windows** — Download artifacts with `curl` through the configured proxy and upload release assets with `curl --noproxy '*'`; `gh run download`/`gh release upload` can hang or be much slower.
