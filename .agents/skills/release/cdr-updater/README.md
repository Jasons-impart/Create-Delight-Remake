...
- 管理员额外加的模组、世界、`eula.txt`、`ops.json` 不会动
- `server.properties` 带 `# cdr-updater-managed`：标签还在且内容被改过则保留；Forge 重生丢掉标签则重新套模板
- 缺失的托管文件会补回
- 私货模组会强制推送；已有 `config/`（TAB、EasyBot 等各服配置）不会被私货盖掉，缺了才会补齐
