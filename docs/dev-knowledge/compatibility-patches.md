...
| 节气覆雪模型 Z-fighting | 模组兼容 | Ecliptic Seasons `0.13.8.1` 提供 `CullTopFaceWithSnow` 原生顶面剔除；旧的模型外扩补丁会与该机制重复，且不再需要。 | `config/eclipticseasons-client.toml` 启用 `CullTopFaceWithSnow`；`kubejs/assets/eclipticseasons/models/block/{overlay,overlay_2,overlay_tiny}.json` 恢复上游微小偏移和 `overlay_2` 的 18 高度。 | 三个 JSON 已通过静态解析；需重启客户端后在冬季覆雪方块及远距离视角回归。 | 节气移除或修改顶面剔除逻辑时，重新评估模型补丁。 | 待游戏内回归 |
