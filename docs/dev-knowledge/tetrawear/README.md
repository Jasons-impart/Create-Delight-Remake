...
```text
a = 12 + 2.5 * damage / (damage + 0.2)
  + 15 * damage / (damage + 18) + 125 * damage / (damage + 750)
taken = a / (a + armor / (1 + armor / 120)
  * (0.7 + 0.3 * min(1, toughness / 10)))
```

该 A-Value 在 0～500 入射伤害内近似原平方根曲线，避开 EvalEx 的 `SQRT` 卡死问题；拟合范围、误差和验证见 [护甲平衡分析](../../attributeslib-armor-balance-analysis.md#已采用的护甲公式)。下表按当前有理式重算，只计算这条护甲公式，不含 Protection、Resistance、吸收、闪避、专属伤害减免、护甲穿透或护甲撕裂：

| 构筑 | Guardian 情况 | 最终护甲 / 韧性 | 40 伤害减免 | 100 伤害减免 | 400 伤害减免 |
|---|---|---:|---:|---:|---:|
| 最高精力 ravager 轻甲 | 期望 6.4 孔 | `93.61 / 32.20` | `62.8%` | `55.6%` | `42.1%` |
| 最高精力 ravager 轻甲 | 十颗上限 | `126.73 / 32.20` | `66.4%` | `59.5%` | `46.0%` |
| Ghost 极限输出流 | 期望 6.4 孔 | `104.94 / 44.97` | `64.2%` | `57.2%` | `43.6%` |
| Ghost 极限输出流 | 十颗上限 | `138.06 / 44.97` | `67.3%` | `60.5%` | `47.0%` |
| Ghost 全板甲 | 期望 6.4 孔 | `118.22 / 49.45` | `65.6%` | `58.7%` | `45.2%` |
| End Dragon 全重甲 | 期望 6.4 孔 | `122.71 / 65.55` | `66.1%` | `59.1%` | `45.6%` |

Ghost 极限输出流有精力时还会增加 `9.7 / 4.55` 主动防御；在期望 Guardian 情况下，Royalty 结算后约为 `116.09 armor / 50.20 toughness`，100 点伤害的护甲减免约 `58.5%`。
