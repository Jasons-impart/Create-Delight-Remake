...
| TACZ-addon × Shoulder Surfing 5.x 开镜兼容 | `com/mafuyu404/taczaddon/compat/ShoulderSurfingCompatInner.badiff` | 将 TACZ-addon 1.1.8-hotfix2 中已移除的 `ShoulderSurfingImpl` 与旧 `api/model/Perspective` 引用迁移到 Shoulder Surfing 5.0.7 的 `ShoulderSurfing` 与 `api/client/Perspective`；保留开镜时的第三人称镜头恢复逻辑。目标 JAR 由 `mods/taczaddon.pw.toml` 和 `packwiz-files/mods/taczaddon-1.20.1-1.1.8-hotfix2-for-new-soph.jar` 固定提供。 |
