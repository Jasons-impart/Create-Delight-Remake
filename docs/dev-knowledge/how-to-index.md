...
| 添加游戏内 Tips | `kubejs/assets/createdelight/lang/zh_cn.json`、`config/tips.json` | Tips 模组从语言键收集提示；CD 自定义提示使用 `createdelight.tip.<id>`，只需新增翻译键，避免把 `createdelight` 加入 `ignoredNamespaces`。 | 校验语言 JSON；进游戏等待 tips 轮播或重载资源后查看。 | 需要批量生成、多语言同步或定制 Tips 过滤规则。 |
