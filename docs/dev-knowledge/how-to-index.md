...
| 维护 GitHub Pages 首页 | `index.html`、`docs/*.html`、`docs/*-data.js`、`.gitignore`、`.github/workflows/release.yml` | Pages 源配置为 `main` 分支根目录；根目录默认被 `.gitignore` 的 `/*` 忽略，新增根页面文件必须加 `!/<file>` allowlist；放在 `docs/` 下的 Pages 内容默认可跟踪且现有发布流程会删除 `docs/`。 | 合并到 `main` 后检查 `https://jasons-impart.github.io/Create-Delight-Remake/`；badge 使用完整 shields.io URL，避免 README/Jekyll 渲染差异；确认 Pages 专用文件未进入服务端包或补丁包。 | 需要自动构建多页站点、发布生成目录或引入主题时再提升。 |
