...
## Release artifact transfer needs split proxy behavior on Windows
...
- **Fix/Lesson**: Use authenticated `curl` for artifact downloads through the configured proxy, use `curl --noproxy '*'` for release asset uploads, and clear artifact extraction directories before retrying.
