 ...
 * @param {string} key 卷轴对应的键（可用于检测）
 * @param {number} material 卷轴身的材质
 * @param {string[]} schematics 卷轴可带来的模块改装
 * @param {number} ribbon 缠绕卷轴的丝带颜色（十六进制）
 * @returns {Internal.ItemStack}
 */
function custom_scroll(glyphs, intricate, key, material, schematics, ribbon) {
    return Item.of("tetra:scroll_rolled", { BlockEntityTag: { data: [{ glyphs: glyphs, intricate: intricate, key: key, material: material, schematics: schematics, ribbon: ribbon }] } })
}
