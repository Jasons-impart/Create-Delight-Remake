...
- 移除/隐藏优先级：物品先用 `StartupEvents.modifyCreativeTab(...).remove(...)` 从创造标签移除，不生效才用 `JEIEvents.hideItems(...)`；无物品形态的流体（如 Create `VirtualFluid` 气体：无方块、无桶、无任何 ItemStack）没有创造标签条目，优先加入 `c:hidden_from_recipe_viewers` 流体 tag 隐藏（JEI 官方机制）；运行时动态隐藏才用 `JEIEvents.hideFluids(...)`。使用 OEI 合并后原物品的隐藏（创造标签、JEI event）由 OEI 预期处理，一般无需在 kubejs 手动隐藏。
...
- Alex's Caves placement is split: magnetic caves (`northstar:moon`), toxic caves (`northstar:venus`), abyssal chasm (`northstar:europa`) and forlorn hollows (`createdelight:lunar_farside`) are direct entries in their dimension `multi_noise` sources; their `../config/alexscaves_biome_generation/*.json` files stay disabled. Alex's Caves only expands the possible-biome list for `minecraft:overworld`, so a non-Overworld generation config can load without making `/locate biome` succeed. `multi_noise` chooses the nearest climate parameter point, not an in-range predicate: a direct entry's climate range alone cannot guarantee either a hard territory boundary or density. Before changing a star-cave band, sample the target dimension's router with the version-matched Misode/Deepslate implementation at a fixed seed. Hard guards require fallback candidates and can replace native cave biomes, so do not add them without an explicit design and overlap analysis. Only primordial caves and candy cavity remain independent dimensions.
